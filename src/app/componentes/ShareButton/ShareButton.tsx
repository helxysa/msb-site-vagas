...
export default function ShareButton({ id, titulo, descricao }: ShareButtonProps) {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      url: `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Confira esta vaga: ${titulo}`)}&url=${encodeURIComponent(shareUrl)}`,
    },
    {
      name: 'LinkedIn',
      icon: (
      ...
      url: `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(titulo)}&summary=${encodeURIComponent(descricao)}`,
    },
  ];
